...
<div class="col-sm-7" style="border-right:1px solid #00B9ED">
    <h2>Edite os dados abaixo</h2>
        @Using Html.BeginForm
            @Html.AntiForgeryToken()
            @Html.ValidationSummary(True)

            @Html.HiddenFor(Function(model) model.id)
            @Html.HiddenFor(Function(model) model.senha)
            @Html.HiddenFor(Function(model) model.ativo)
            @Html.HiddenFor(Function(model) model.dataCadastro)
            @Html.HiddenFor(Function(model) model.dataPenultimoAcesso)
            @Html.HiddenFor(Function(model) model.dataUltimoAcesso)
            @Html.HiddenFor(Function(model) model.idFacebook)     
            @Html.HiddenFor(Function(model) model.idGoogle)
            @Html.HiddenFor(Function(model) model.linkFacebook)
...
<div class="col-sm-5">
    ...
    <!-- CDO-EditarPerfil -->
    <ins class="adsbygoogle cdo-editarperfil"
         style="display:inline-block"
         data-ad-client="ca-pub-1017535286769229"
         data-ad-slot="2248470439"></ins>
    <script>
